...
void  OSSemCreate (OS_SEM      *p_sem, // 指向信号量控制块
                   CPU_CHAR    *p_name,
                   OS_SEM_CTR   cnt,  // 设置信号量的初始值，如果此值为 1，代表此信号量为二进制信号量，如果大于 1 的话就代表此信号量为计数型信号量。
                   OS_ERR      *p_err)
{
    CPU_SR_ALLOC();
...
OS_SEM_CTR  OSSemPend (OS_SEM   *p_sem,  // 指向一个信号量的指针
                       OS_TICK   timeout,   //超出时间
                       OS_OPT    opt,       //是否设置阻塞模式
                       CPU_TS   *p_ts,    // 指向一个时间戳，用来记录接收到信号量的时刻
                       OS_ERR   *p_err)   // 保存调用函数的错误代码
{
    OS_SEM_CTR    ctr;
...
OS_SEM_CTR  OSSemPost (OS_SEM  *p_sem,   //发送信号量   释放
                       OS_OPT   opt,
                       OS_ERR  *p_err)
{
    OS_SEM_CTR  ctr;
